...
parser$add_argument("--num_folds", default=1, type="integer",
                    help="folds count")

parser$add_argument("--group", default='9', type="character",
                    help="folds count")
...
parser$add_argument("--output_path_all", default='/Users/axr2376/Desktop/pred_1_0_paper/data_out/expt_1/kfold/state_probs/all_states/', type="character",
                    help="all states")
...
for (fold in 1: num_folds-1){
    preds_groups_per_fold = paste('test_group_', group, '_fold_', fold, '_', model, sep='')
    model_indiv_pred_files = list.files(path=base_path, pattern=preds_groups_per_fold)

    prog_p1 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Progress (P1).csv', sep = ''))[-1,]
    prog_dec_p1 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Progress & deceased (P1).csv', sep = ''))[-1,]
    move_to_phase_2 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Move to phase 2.csv', sep = ''))[-1,]
    prog_p2 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Progress (P2).csv', sep = ''))[-1,]
    prog_dec_p2 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Progress & deceased (P2).csv', sep = ''))[-1,]
    non_prog_p2 = read.csv(paste(base_path, 'test_group_', group, '_fold_', fold, '_', model, '_Non-progress & deceased (P2).csv', sep = ''))[-1,]
    ...
    for (ind in 1: dim(prog_p1)[2]) {
      ...
      if (group == '9' && model == 'neural_cox_non_prop') {
        write.csv(SaveData,row.names = FALSE,file=paste(output_path_all, 'testGroup~hd~', Sample, '~fold~', fold, '~', model, '.csv', sep = ''))
      }
      # OS
    ...
    write.csv(adj_Data2OS,row.names = FALSE,file=paste(output_path_os,'testgroup-', group, '-fold-', fold, '-', model,'.csv',sep=''))
    
    # EFS
    colnames(adj_Data2EFS) = adj_Data2EFS[1,]
    adj_Data2EFS = adj_Data2EFS[-1, ]
    write.csv(adj_Data2EFS,row.names = FALSE,file=paste(output_path_efs,'testgroup-', group, '-fold-', fold, '-', model,'.csv',sep=''))
