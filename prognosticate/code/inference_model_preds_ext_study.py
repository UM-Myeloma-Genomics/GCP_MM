...
parser = argparse.ArgumentParser(description='Train on validation cohort and test on independent study')
parser.add_argument('--seed', type=int, default=456, help='random seed (default: 42)')
parser.add_argument('--kfolds', type=int, default=5, help='Cross validation, number of folds')
...
def cph(train_x, train_y, test_x, val_x_bystate, val_y_bystate):
    ...
    surv_test_prob = np.row_stack([fn(times_ytrain) for fn in cph.predict_survival_function(test_x_std_norm)])
    surv_test = pd.DataFrame(surv_test_prob.T, index=times_ytrain).astype('float32')
    surv_test.index.names = ['duration']

    return surv_test, c_ind_train, c_ind_val, cph
...
def rsf(train_x, train_y, test_x, val_x_bystate, val_y_bystate, model_param):
    ...
    surv_test = model.predict_survival_function(test_x, return_array=True)
    surv_test = pd.DataFrame(surv_test.T)
    surv_test.index = model.event_times_
    surv_test.index.names = ['duration']
    ...
    return surv_test, c_ind_train, c_ind_val, model
...
def cox_time(train_x, train_y, test_x, val_x_bystate, val_y_bystate, cols_x, model_param, group, df_test):
    ...
    df_shap_mean = None ; df_test_shap = None ; c_ind_train = None ; c_ind_val = None
...
def train_survival_(fold, train_x, train_y, val_x, val_y, test_x, df_test, group, model_param):
    ...
    if args.surv_model == 'cph':
        test_surv_df, c_ind_train, c_ind_val, estimator = cph(train_x, df_train_y, test_x, val_x, df_val_y_bystate)

    if args.surv_model == 'rsf':
        test_surv_df, c_ind_train, c_ind_val, estimator = rsf(train_x, df_train_y, test_x, val_x, df_val_y_bystate, model_param)

    if args.surv_model == 'neural_cox_non_prop':
        test_surv_df, c_ind_train, c_ind_val, estimator, mean_shap_values, df_test_shap = \
                                                    cox_time(train_x, df_train_y, test_x, val_x, df_val_y_bystate,
                                                             cols_x, model_param, group, df_test)

    # interpolate
    idx = np.arange(0, 2554, 1)
    df_interp_pred = pd.merge(test_surv_df, pd.DataFrame(index=idx), left_index=True, right_index=True, how='outer')
...
def phase_1(fold, train_x, train_y, val_x, val_y, test_x, test_y, df_train, df_val, df_test, model_param_list, dict_pred_probs):
    # print('phase 11', train_y['os_time'].min(), train_y['os_time'].max())

    # G1 Phase 1 induction (progressors vs non-progressors)
    df_pred_probs_prog_p1, gp_prog_p1 = progress_p1(fold, train_x, train_y, val_x, val_y, test_x, test_y, df_train, df_val, df_test, model_param_list)
    ...
    df_pred_probs_prog_dec_p1, gp_prog_dec_p1 = progress_dec_p1(fold, train_x, train_y, val_x, val_y, test_x, test_y, df_train, df_val, df_test, model_param_list)
    dict_pred_probs[gp_prog_dec_p1] = df_pred_probs_prog_dec_p1
    # print('phase 13', train_y['os_time'].min(), train_y['os_time'].max())

    # G2 phase 2 vs 1
    df_pred_probs_to_p2, gp_to_p2 = move_to_phase2(fold, train_x, train_y, val_x, val_y, test_x, test_y, df_train, df_val, df_test, model_param_list)
    dict_pred_probs[gp_to_p2] = df_pred_probs_to_p2

    return dict_pred_probs


def phase_2(fold, train_x, train_y, val_x, val_y, test_x, test_y, df_train, df_val, df_test, model_param_list, dict_pred_probs):
    ...
    df_pred_probs_prog_p2, gp_prog_p2 = progress_p2(fold, X_p2, Y_p2, val_x, val_y, test_x, test_y, df_p2, df_val, df_test, model_param_list)
    ...
    df_pred_probs_prog_dec_p2, gp_prog_dec_p2 = progress_dec_p2(fold, X_p2, Y_p2, val_x, val_y, test_x, test_y, df_p2, df_val, df_test, model_param_list)
    ...
    df_pred_probs_non_prog_dec_p2, gp_non_prog_dec_p2 = non_progress_dec_p2(fold, X_p2, Y_p2, val_x, val_y, test_x, test_y, df_p2, df_val, df_test, model_param_list)
...
def patient_cross_val(model_param_list):
    ...
    for train_index, val_index in rskf.split(X, x_df_copy['unique_patient_groups']):
        if fold == 0: # choose a split of indices for running through the independent test set once
            ...
            ########## Phase 1 groups ################
            dict_pred_probs = phase_1(fold, train_x, train_y, val_x, val_y, test_x, test_y,
                                            x_df_train, x_df_val, x_df_test, model_param_list, dict_pred_probs)

            ########## Phase 2 groups ################
            dict_pred_probs = phase_2(fold, train_x, train_y, val_x, val_y, test_x, test_y, x_df_train, x_df_val,
                                x_df_test, model_param_list, dict_pred_probs)
            ...
            for ind, (k, v) in enumerate(dict_pred_probs.items()):
                    ...
                    v.to_csv(args.path + '/predictions_test_set/model_probs/test_group_' + group_id + '_fold_' + str(fold) + '_' +
